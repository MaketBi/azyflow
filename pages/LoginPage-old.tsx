...
export const LoginPage: React.FC = () => {
  ...
  // Charger les sociétés existantes pour les freelancers
  useEffect(() => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isSignUp) {
      ...
      // Créer le compte avec les métadonnées dans l'auth
      if (role === 'admin') {
        if (!companyName.trim()) {
          setError('Le nom de la société est requis pour un administrateur');
          setLoading(false);
          return;
        }
        ...
        if (error) {
          ...
          setSuccess(`Compte administrateur créé avec succès ! Vous pourrez configurer votre société "${companyName}" lors de votre première connexion. Vérifiez votre email pour confirmer votre compte.`);
          // Reset form
          setEmail('');
          setPassword('');
          setFullName('');
        ...
        // Si c'est un freelancer, utiliser la méthode normale
        if (!companyId) {
      ...
      if (error) {
        ...
        // Connexion réussie - le trigger a déjà créé le profil si nécessaire
  ...
  // Si on doit afficher la page de setup de profil
  if (showProfileSetup && setupUserData) {
    return (
      <ProfileSetupPage 
